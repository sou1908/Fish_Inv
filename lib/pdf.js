...
// Build the client-side P&L report (fish.md §5).
export function buildReport({ settings, from, to, summary, byProduct, daily, dow, purchases, wastage, closingStock }) {
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      ["COGS (cost of goods sold)", money(summary.cogs)],
      ["Gross profit", money(summary.grossProfit)],
      ["Overheads", money(summary.overheads)],
  ...
  if (daily.length) {
    autoTable(doc, {
      ...
      body: daily.map((d) => [prettyDate(d.date), money(d.revenue), money(d.profit)]),
      theme: "grid",
      headStyles: { fillColor: [13, 148, 136] },
    });
    y = doc.lastAutoTable.finalY + 8;
